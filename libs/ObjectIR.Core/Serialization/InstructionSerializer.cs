...
public sealed class InstructionSerializer
{
    ...
    private static InstructionData CreateInstructionData(Instruction instruction)
    {
        return instruction switch
        {
            ...
            NewObjectInstruction noi => new InstructionData
            {
                ...
                Operand = new
                {
                    type = noi.Type.GetQualifiedName(),
                    parameterTypes = noi.ParameterTypes.Select(p => p.GetQualifiedName()).ToArray()
                }
            },
    ...
    public static Instruction DeserializeInstruction(JsonElement element)
    {
        ...
        return opCode switch
        {
            ...
            "newobj" => new NewObjectInstruction(
                TypeReference.FromName(GetString(operand, "type")),
                GetStringArray(operand, "parameterTypes").Select(TypeReference.FromName).ToList()
            ),
    ...
    private static IEnumerable<string> GetStringArray(JsonElement element, string propertyName)
    {
        ...
        if (prop.ValueKind != System.Text.Json.JsonValueKind.Array)
            return Enumerable.Empty<string>();
        return prop.EnumerateArray().Select(e => e.GetString() ?? string.Empty);
    }
